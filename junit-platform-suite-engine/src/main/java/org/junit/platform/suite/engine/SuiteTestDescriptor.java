...
final class SuiteTestDescriptor extends AbstractTestDescriptor {
	...
	private static String getSuiteDisplayName(Class<?> suiteClass, DiscoveryIssueReporter issueReporter) {
		// @formatter:off
		Predicate<String> nonBlank = issueReporter.createReportingCondition(StringUtils::isNotBlank, __ -> {
			String message = String.format("@SuiteDisplayName on %s must be declared with a non-blank value.",
					suiteClass.getName());
